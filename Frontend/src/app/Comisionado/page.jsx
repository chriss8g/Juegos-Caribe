...
export default function Commissioner()
{
    const commissioner = [
        ...
        // Paso 1: Dividir la lista commissioner
    ...
    useEffect(() => {
        ...
        const organizativaList = commissioner.filter(item => item.comision === 'organizativa').map(item => ({
            name: item.name,
            body: item.cargo,
            image: item.image
        }));

        const disciplinariaPList = commissioner.filter(item => item.comision === 'disciplinariaP').map(item => ({
            name: item.name,
            body: item.cargo,
            image: item.image
        }));

        const disciplinariaSList = commissioner.filter(item => item.comision === 'disciplinariaS').map(item => ({
            name: item.name,
            body: item.cargo,
            image: item.image
        }));
